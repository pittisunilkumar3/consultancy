(function ($) {
    ...
    $(document).on('keyup', "#search-key", function () {
        ...
        debounceTimer = setTimeout(function () {
            let url = updateUrlWithSearchKey($('#my-service-route').val(), $('#search-key').val());
            searchCourses(url);
        }, 300);
    });
    ...
    function searchResponse(response) {
        if (response.status === 200) {
            $('#service-list-container').html(response.responseText);

            let currentSearchKey = $('#search-key').val();
            $('#service-list-container .pagination a').each(function () {
                let newUrl = updateUrlWithSearchKey($(this).attr('href'), currentSearchKey);
                $(this).attr('href', newUrl);
            });
        } else {
            $('#service-list-container').html('');
        }
    }
    ...
    $(document).on('click', '#addFeature', function () {
        let newFeature = $('.feature-item').first().clone();

        let uniqueId = Date.now();

        newFeature.find('[id]').each(function () {
            let oldId = $(this).attr('id');
            let newId = oldId + '_' + uniqueId;
            $(this).attr('id', newId);

            newFeature.find(`label[for="${oldId}"]`).attr('for', newId);
        });

        newFeature.find('input').val('');
        newFeature.find('.text-danger').remove();
        newFeature.find('.is-invalid').removeClass('is-invalid');

        if (!newFeature.find('.removeFeature').length) {
            newFeature.find('.answer-block').append(`
            <button type="button" class="removeFeature top-0 end-0 bg-transparent border-0 p-0 me-2 rounded-circle d-flex justify-content-center align-items-center position-absolute">
