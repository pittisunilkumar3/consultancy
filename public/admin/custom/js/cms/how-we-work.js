(function ($) {
    ...
    $('.addHowWeWorkBtn').on('click', function () {
        let newBlock = $('.howWeWorkItemsContainer').first().clone();

        newBlock.find('input').val('');

        newBlock.find('.removeHowWeWorkBtn').remove();
        newBlock.find('.col-6:last-child').append(`
        ...
        $('#howWeWorkContainer').append(newBlock);
    });

    $('#howWeWorkContainer').on('click', '.removeHowWeWorkBtn', function () {
        $(this).closest('.howWeWorkItemsContainer').remove();
    });
