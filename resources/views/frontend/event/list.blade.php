...
@section('content')
    ...
    <section class="inner-breadcrumb-section">
        <div class="container">
            ...
            <h4 class="inner-breadcrumb-title">{{__('Event')}}</h4>
            ...
            <div class="row rg-24 align-items-center pb-md-40 pb-20" data-aos="fade-up" data-aos-duration="1000">
                <div class="col-lg-8">
                    <div class="align-items-center course-search-suggestion">
                        ...
                        <span class="fs-18">
                            {{ ($eventData->currentPage() - 1) * $eventData->perPage() + 1 }}
                            -
                            {{ min($eventData->currentPage() * $eventData->perPage(), $eventData->total()) }}
                            {{__('of')}}
                            {{ $eventData->total() }} {{__('events')}}
                        </span>
                    </div>
                </div>
                <div class="col-lg-4">
                    <form id="sortForm" method="GET" action="{{ route('event.list') }}">
                            ...
                            <div class="course-search-sort">
                                <div class="d-flex align-items-center g-10">
                                    ...
                                    <select name="sort_by" class="sf-select-checkbox"
                                        ...
                                        <option
                                            value="title-asc" {{ request('sort_by') == 'title-asc' ? 'selected' : '' }}>{{__('Name A to Z')}}</option>
                                        <option
                                            value="title-desc" {{ request('sort_by') == 'title-desc' ? 'selected' : '' }}>{{__('Name Z to A')}}</option>
                                    </select>
                                </div>
                            </div>
                        </div>
            ...
            <div class="row rg-24">
                <div class="col-lg-4 col-md-6">
                    <div class="course-search-sidebar" data-aos="fade-up" data-aos-duration="1000">
                        ...
                        <form id="filterForm" method="GET" action="{{ route('event.list') }}">
                                ...
                                <div class="swiper-wrapper">
                                    <div class="swiper-slide">
                                        <div class="course-item-three">
                                            <a href="{{route('event.details',$event->slug)}}" class="img">
                                                <img src="{{ getFileUrl($event->image) }}" alt=""/>
                                                <p class="dateTime">{{ \Carbon\Carbon::parse($event->date_time)->format('M jS Y , g:i A') }}</p>
                                            </a>
                                            <div class="course-content">
                                                <ul class="category-list">
                                                    @if($event->type == EVENT_TYPE_PHYSICAL)
                                                        <li><p class="item">{{__('Physical')}}</p></li>
                                                    @elseif($event->type == EVENT_TYPE_VIRTUAL)
