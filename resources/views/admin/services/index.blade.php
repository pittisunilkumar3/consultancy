...
@section('content')
    <div class="px-sm-30 p-15 bd-b-one bd-c-stroke-2 d-flex justify-content-between align-items-center">
        ...
        <div>
            <a href="{{ route('admin.services.create') }}" class="flipBtn sf-flipBtn-primary flex-shrink-0">+ {{__('Add Service')}}</a>
        </div>
    </div>
    <div class="p-15 p-sm-30" id="service-list-container">
        @include('admin.services.partials.list')
    </div>
    <input type="hidden" id="my-service-route" value="{{ url()->current() }}">
@endsection
@push('script')
