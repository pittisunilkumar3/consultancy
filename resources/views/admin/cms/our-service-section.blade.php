                ...
                <div class="col-xl-9">
                    ...
                    <div class="bg-white p-sm-25 p-15 bd-one bd-c-stroke bd-ra-8">
                        <form class="ajax" action="{{ route('admin.setting.application-settings.update') }}"
                            ...
                            @csrf
                            <div class="align-items-center bd-b-one bd-c-one bd-c-stroke bd-c-stroke-2 d-flex justify-content-between mb-30 pb-15 pb-sm-30">
                                <h4 class="fs-18 fw-700 lh-24 text-title-text">{{__('Our Service Image')}}</h4>
                            </div>
                            <div class="row">
                                <div class="col-xl-12 col-md-12 col-sm-12">
                                    <div class="primary-form-group">
                                        <div class="primary-form-group-wrap zImage-upload-details">
                                            ...
                                            <label class="zForm-label-alt">{{ __('Our Service Image') }} <span
                                                    class="text-danger">*</span></label>
                                            <div class="upload-img-box">
                                                @if (getOption('our_service_image'))
                                                    <img src="{{ getSettingImage('our_service_image') }}" />
                                                @else
                                                    <img src="" />
                                                @endif
                                                <input type="file" name="our_service_image" id="zImageUpload"
                                                       accept="image/*" onchange="previewFile(this)" />
                                            </div>
                                        </div>
                                    </div>
                                    @if ($errors->has('our_service_image'))
                                        <span class="text-danger"><i class="fas fa-exclamation-triangle"></i>
                                                {{ $errors->first('our_service_image') }}</span>
                                    @endif
                                    <p>
