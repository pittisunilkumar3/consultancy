            ...
            <div class="col-xl-12">
                <div class="bd-one bd-c-stroke bd-ra-8 bd-bl-ra-0 bd-br-ra-0 pt-25 px-sm-25 px-10 bg-white">
                    <ul class="d-flex flex-wrap rg-20 zList-four">
                        <li>
                            <a href="{{ route(getPrefix().'.profile.index') }}"
                        ...
                        <li>
                            <a href="{{ route(getPrefix().'.profile.password') }}"
                ...
                <form class="ajax" action="{{ route(getPrefix().'.profile.update') }}" method="POST"
                    ...
                    <div class="p-sm-25 p-15 bd-one bd-t-zero bd-c-stroke bd-ra-8 bd-tl-ra-0 bd-tr-ra-0 bg-white mb-25">
                        <h4 class="fs-18 fw-600 lh-22 text-title-text pb-25">{{ __('Security') }} :</h4>
                        <div class="row pb-20 bd-b-one bd-c-stroke col-md-12 pb-20 mb-20 mx-0">
                            <div class="col-md-6 mx-0 px-0">
                                <ul class="securityList">
                                    <li class="d-flex justify-content-between align-items-center">
                                        <div class="">
                                            <h4 class="fs-18 fw-500 lh-28">
                                                {{__('Google/Microsoft Authentication (Recommended)')}}</h4>
                                            <p class="fs-14 fw-400 lh-22">
                                                {{__('Protect your account and transactions.')}}</p>
                                        </div>

                                        <div class="zCheck form-switch">
                                            <input id="googleAuthCheckbox" class="form-check-input mt-0"
                                                   {{ auth()->user()->google_auth_status == STATUS_ACTIVE ? 'checked' : '' }}
                                                   type="checkbox" role="switch"
                                                   onclick="event.preventDefault(); if('{{ auth()->user()->google_auth_status }}' == 1) { new bootstrap.Modal(document.getElementById('googleAuthDisableModal')).show(); } else { new bootstrap.Modal(document.getElementById('googleAuthEnableModal')).show(); }">
                                        </div>
                                    </li>
                                </ul>
                            </div>
                        </div>
                        <h4 class="fs-18 fw-600 lh-22 text-title-text pb-25">{{ __('Personal Information') }} :</h4>
                        <div class="pb-40">
                        ...
                        <div class="row rg-20 pb-40">
                            <div class="col-md-6">
                                <label for="editProfileName" class="zForm-label-alt">{{ __('First Name') }}</label>
                                <input type="text" name="first_name" class="form-control zForm-control-alt" id="editProfileName"
                                       value="{{ auth()->user()->first_name }}" placeholder="{{ __('First Name') }}"/>
                            </div>
                            <div class="col-md-6">
                                <label for="editProfileLastName" class="zForm-label-alt">{{ __('Last Name') }}</label>
                                <input type="text" name="last_name" class="form-control zForm-control-alt" id="editProfileLastName"
                                       value="{{ auth()->user()->last_name }}" placeholder="{{ __('Last Name') }}"/>
                            </div>
                            <div class="col-md-6">
                                <label for="editProfileEmail" class="zForm-label-alt">{{ __('Email') }}</label>
                                <input type="text" readonly name="email" class="form-control zForm-control-alt"
                                       value="{{ auth()->user()->email }}" id="editProfileEmail"
                                       placeholder="{{ __('Email') }}"/>
                            </div>
                            <div class="col-md-6">
                                <label for="editProfileCountry" class="zForm-label-alt">{{ __('Designation') }}</label>
                                <select class="sf-select-two" name="designation_id">
                                    <option value="">{{ __('Select') }}</option>
                                    @foreach ($designations as $designation)
                                        <option value="{{ $designation->id }}" {{ auth()->user()->designation_id ==
                            ...
                            <div class="d-flex g-12">
                                <button
                                    class="flipBtn sf-flipBtn-primary flex-shrink-0">{{
                            __('Update') }}</button>
                            </div>
                        </div>
                    </div>
    ...
    <div class="modal fade zModalOne" id="googleAuthEnableModal" tabindex="-1"
        ...
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content zModalOne-content">
                <div class="bd-b-one bd-c-stroke px-16 py-20 mb-20 d-flex align-items-center flex-wrap justify-content-between g-10">
                    <h2 class="fs-18 fw-600 lh-22 text-title-text">{{ __('Enable 2FA') }}</h2>
                    <div class="mClose">
                        <button type="button"
                ...
                <div class="modal-body zModalOne-body">
                    ...
                    <p class="fs-14 fw-400 lh-22 text-707070 pb-6"><span class="text-font-black fw-700">{{__('Step 1')}}
                                    :</span> {{__('Install this app from')}} <a
                            href="https://play.google.com/store/apps/details?id=com.google.android.apps.authenticator2"
                            target="__blank"
                            class="text-font-black text-decoration-underline fw-700">{{__('google play store')}}</a> {{__('or')}}
                        <a href="https://itunes.apple.com/us/app/google-authenticator/id388497605"
                           target="__blank"
                           class="text-font-black text-decoration-underline fw-700">{{__('App store')}}</a></p>
                    <p class="fs-14 fw-400 lh-22 pb-10"><span
                            class="text-font-black fw-700">{{__('Step 2 ')}}:</span> {{__('Scan the below QR code by your google/microsoft authenticator app, or you can add account manually.')}}
                    </p>
                    <p class="fs-14 fw-700 lh-22 text-font-black pb-10">{{__('Manually Add Account:')}}</p>
                    <p class="fs-14 fw-700 lh-22 text-font-black pb-10">{{__('Account Name : ')}} {{ getOption('app_name') }}</p>
                    <p class="fs-14 fw-700 lh-22 text-font-black pb-20">{{__('Key')}}
                        ...
                        <div class="row rg-20">
                            <div class="col-md-12">
                                <label for="authenticationCode"
                                       class="zForm-label-alt">{{__('Enter google/microsoft authenticator code')}}</label>
                            ...
                            <div class="col-md-12">
                                <button
                                    class="sf-btn-primary">{{__('Confirm 2FA')}}</button>
                            </div>
                        </div>
                    </form>
                    <p class="fs-15 fw-500 lh-25 text-ea4335 pt-20">{{__('Note : If you lost your phone or uninstall the
                                google/microsoft authenticator app, then you will lost access of your account.')}}</p>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content zModalOne-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">{{ __('Enable 2FA') }}</h5>
                                ...
                                <div class="pb-20 pt-10 primary-form-group">
                                    <div class="primary-form-group-wrap">
                                        <label for="authenticationCodeDisable"
                                               class="zForm-label-alt">{{ __('Enter google/microsoft authenticator
                                                                                                                                                                                                                                                                                                                                                                                                                                                    code') }}</label>
                                        <input type="text" class="form-control zForm-control-alt" name="one_time_password"
                                               id="authenticationCodeDisable"
                                               placeholder="{{ __('Enter the code to verify') }}"/>
                                    </div>
                                </div>
                                <button type="submit"
                                        class="flex-shrink-0 flipBtn sf-btn-brand">{{ __('Disable 2FA') }}</button>
                            </form>
                        </div>
                    </div>
                </div>
