...
@section('content')
        ...
        <div class="row rg-20">
            ...
            <div class="col-xl-9">
                <div class="align-items-center bd-b-one bd-c-one bd-c-stroke bd-c-stroke-2 d-flex justify-content-between mb-30 pb-15 pb-sm-30">
                    ...
                    <button data-bs-toggle="modal" data-bs-target="#add-modal" class="flipBtn sf-flipBtn-primary flex-shrink-0">{{ __('+ Add New') }}</button>
                </div>
                <div class="p-sm-25 p-15 bd-one bd-c-stroke bd-ra-10 bg-white">
                    <table class="table zTable zTable-last-item-right" id="certificationDataTable">
                        <thead>
                            <tr>
                                <th>
                                    <div>{{ __('Title') }}</div>
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content border-0 bd-ra-4 p-sm-25 p-15">
                <div class="bd-b-one bd-c-stroke pb-20 mb-20 d-flex align-items-center flex-wrap justify-content-between g-10">
                    <h2 class="fs-18 fw-600 lh-22 text-title-text">{{ __('Add Certification & Degree') }}</h2>
                    <div class="mClose">
                        <button type="button"
                ...
                <form class="ajax reset" action="{{ route('admin.setting.certificate_degrees.store') }}" method="post"
                      data-handler="commonResponseForModal">
                    @csrf
                    <div class="row rg-20 mb-20">
                        <div class="col-12">
                            <label for="addTitle" class="zForm-label-alt">{{ __('Title') }}</label>
                            <input type="text" name="title" class="form-control zForm-control-alt" id="addTitle"
    ...
    <input type="hidden" id="designationIndexRoute" value="{{ route('admin.setting.certificate_degrees.index') }}">
@endsection

@push('script')
    <script src="{{ asset('admin/custom/js/certificate_degrees.js') }}"></script>
@endpush
