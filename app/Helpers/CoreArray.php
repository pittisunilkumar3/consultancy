...
if (!function_exists("getRoleName")) {
    function getRoleName($input = null)
    {
        $output = [
            ...
            USER_ROLE_STAFF => __('Staff'),
            USER_ROLE_CONSULTANT => __('Consultant'),
        ];
...
if (!function_exists("offDays")) {
    function offDays($input = null)
    {
        $output = [
            1 => __('Sunday'),
            2 => __('Monday'),
        ...
        if (is_null($input)) {
            return $output;
        } elseif (isset($output[$input])) {
            return $output[$input];
        } else {
            return __('Invalid day');
        }
    }
}


if (!function_exists("dayShort")) {
    function dayShort($input = null)
    {
        $output = [
            1 => __('Sun'),
            2 => __('Mon'),
...
if (!function_exists("getPathByRole")) {
    function getPathByRole()
    {
        $input = auth()->user()->role;
        $output = [
            USER_ROLE_ADMIN => 'admin',
            USER_ROLE_STUDENT => 'student',
            USER_ROLE_STAFF => 'admin',
        ];
...
function getPaymentServiceClass($input = null)
{
    $output = array(
        ...
        CASH => 'App\Http\Services\Payment\CashService',  // Add the bank service here
    );
    if (is_null($input)) {
        return $output;
    } else {
...
if (!function_exists("getEventMeetingType")) {
    function getEventMeetingType($input = null)
    {
        $output = [
            EVENT_TYPE_PHYSICAL => __('Physical'),
            EVENT_TYPE_VIRTUAL => __('Virtual')
        ];
...
if (!function_exists("getResourceType")) {
    function getResourceType($input = null)
    {
        $output = [
            RESOURCE_TYPE_LOCAL => __('Video'),
            RESOURCE_TYPE_YOUTUBE_ID => __('Youtube'),
            RESOURCE_TYPE_PDF => __('PDF'),
            RESOURCE_TYPE_IMAGE => __('Image'),
            RESOURCE_TYPE_AUDIO => __('Audio'),
            RESOURCE_TYPE_SLIDE => __('Slide'),
        ];
...
if (!function_exists("getTransactionType")) {
    function getTransactionType($input = null)
    {
        $output = [
            TRANSACTION_EVENT => __('Event Booking'),
            TRANSACTION_COURSE => __('Course Purchase'),
            TRANSACTION_CONSULTATION => __('Consultation Booking'),
            TRANSACTION_SERVICE => __('Service Order'),
            TRANSACTION_INVOICE => __('Invoice Payment'),
            TRANSACTION_OTHERS => __('Others'),
        ];
...
if (!function_exists("getFundType")) {
    function getFundType($input = null)
    {
        $fundTypes = [
            FUND_TYPE_SELF => __('Self-Funded'),
            FUND_TYPE_PARENTS => __('Parents'),
            FUND_TYPE_SEEKING_SCHOLARSHIP => __('Seeking Scholarship'),
            FUND_TYPE_BANK_LOAN => __('Bank Loan'),
            FUND_TYPE_OTHER => __('Other'),
            FUND_TYPE_EMPLOYER_SCHOLARSHIP => __('Employer Scholarship'),
        ];

        if (is_null($input)) {
            return $fundTypes;
        } else {
            return $fundTypes[$input] ?? __('Unknown');
        }
    }
}


if (!function_exists("getConsultationType")) {
    function getConsultationType($input = null)
    {
        $output = [
            CONSULTATION_TYPE_PHYSICAL => __('In Person'),
            CONSULTATION_TYPE_VIRTUAL => __('Virtual')
        ];
...
if (!function_exists("getMeetingPlatform")) {
    function getMeetingPlatform($input = null)
    {
        $output = [
            MEETING_PLATFORMS_PERSON => __('In Person'),
            MEETING_PLATFORMS_MEET => __('Google Meet'),
            MEETING_PLATFORMS_ZOOM => __('Zoom'),
        ];
...
if (!function_exists("getStatusHtml")) {
    function getStatusHtml($input = null)
    {
        $output = [
            STATUS_ACTIVE => '<span class="zBadge zBadge-active">' . __('Active') . '</span>',
            STATUS_DEACTIVATE => '<span class="zBadge zBadge-deactive">' . __('Deactivate') . '</span>',
            STATUS_PENDING => '<span class="zBadge zBadge-pending">' . __('Pending') . '</span>',
        ];
...
if (!function_exists("getPriority")) {
    function getPriority($input = null)
    {
        $output = [
            ORDER_TASK_PRIORITY_LOWEST => __('Lowest'),
            ORDER_TASK_PRIORITY_LOW => __('Low'),
            ORDER_TASK_PRIORITY_MEDIUM => __('Medium'),
            ORDER_TASK_PRIORITY_HIGH => __('High'),
            ORDER_TASK_PRIORITY_HIGHEST => __('Highest'),
        ];
...
if (!function_exists("getPriorityClass")) {
    ...
    {
        switch ($priority) {
            case ORDER_TASK_PRIORITY_LOWEST:
                return 'text-info';
            case ORDER_TASK_PRIORITY_LOW:
                return 'text-main-color';
            case ORDER_TASK_PRIORITY_MEDIUM:
                return 'text-purple';
            case ORDER_TASK_PRIORITY_HIGH:
                return 'text-danger';
            case ORDER_TASK_PRIORITY_HIGHEST:
                return 'text-red';
            default:
                return '';
        }
...
if (!function_exists("getOrderTaskStatus")) {
    function getOrderTaskStatus($input = null)
    {
        $output = [
            ORDER_TASK_STATUS_PENDING => __('Pending'),
            ORDER_TASK_STATUS_PROGRESS => __('Progress'),
            ORDER_TASK_STATUS_REVIEW => __('Review'),
            ORDER_TASK_STATUS_DONE => __('Done'),
        ];
...
if (!function_exists("getOrderStatusName")) {
    function getOrderStatusName($input = null)
    {
        $output = [
            ...
            WORKING_STATUS_CANCELED => __('Canceled'),
        ];
...
if (!function_exists("getResourceIcon")) {
    function getResourceIcon($input = null)
    {
        $output = [
            RESOURCE_TYPE_LOCAL => 'resource-video.svg',
            RESOURCE_TYPE_YOUTUBE_ID => 'resource-video.svg',
            RESOURCE_TYPE_PDF => 'resource-pdf.svg',
            RESOURCE_TYPE_IMAGE => 'resource-image.svg',
            RESOURCE_TYPE_AUDIO => 'resource-audio.svg',
            RESOURCE_TYPE_SLIDE => 'resource-slide.svg',
        ];
